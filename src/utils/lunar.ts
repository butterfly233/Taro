...
export const calendar = {
   ...
   */
  solar2lunar: function (year: any, month: any = 0, day: any = 0) { //参数区间1900.1.31~2100.12.31
    if (year < 1900 || year > 2100) { return -1; }//年份限定、上限
    if (year == 1900 && month == 1 && day < 31) { return -1; }//下限

    let objDate = year ? new Date(year, parseInt(month) - 1, day) : new Date();  //未传参 获得当天
    let i, temp = 0;
    //修正ymd参数
    ...
    for (i = 1; i < 13 && offset > 0; i++) {
      ...
      if (leap > 0 && i == (leap + 1) && isLeap == false) {
        --i;
        isLeap = true; temp = calendar.leapDays(lunarYear); //计算农历闰月天数
      }
      else {
    ...
    return { 'lYear': year, 'lMonth': month, 'lDay': day, 'Animal': calendar.getAnimal(year), 'IMonthCn': (isLeap ? "\u95f0" : '') + calendar.toChinaMonth(lunarMonth), 'IDayCn': calendar.toChinaDay(lunarDay), 'cYear': y, 'cMonth': m, 'cDay': d, 'gzYear': gzY, 'gzMonth': gzM, 'gzDay': gzD, 'isToday': isToday, 'isLeap': isLeap, 'nWeek': nWeek, 'ncWeek': "\u661f\u671f" + cWeek, 'isTerm': isTerm, 'Term': Term, 'astro': astro };
  },
  /**
